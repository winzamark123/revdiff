...
Zed's [custom tasks](https://zed.dev/docs/tasks) can launch revdiff directly in an editor terminal tab. When `$EDITOR` resolves to `zed` or `zeditor`, pressing `e` opens the review's workspace root and focused file together, so Zed starts with the project tree available instead of an isolated file. Use `EDITOR=zed` for this non-blocking browsing flow; `zed --wait` waits until the workspace window closes because revdiff opens a directory.
